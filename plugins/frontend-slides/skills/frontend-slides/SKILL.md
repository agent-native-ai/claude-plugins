...
| 状況 | 対応 |
|------|------|
| ユーザーが12プリセットのどれにも満足しない | ユーザーに確認: 「参考にしたいWebサイトやデザインのURLを教えてください」 |
| PPT内の画像が破損・欠損 | ユーザーに確認: 「{N}枚の画像が抽出できませんでした。元ファイルを確認してください」 |
| スライド数が30枚超 | ユーザーに確認: 「パフォーマンス影響があります。分割を推奨しますか？」 |
...
そのまま使える。Configの値（preview_dir, フォントサイズ等）は全て汎用デフォルト。企業名ハードコードなし。

ブランド固有のデザインシステムがある場合は、プロジェクトの `.claude/rules/` にブランド適用ルールを追加することで対応可能。
