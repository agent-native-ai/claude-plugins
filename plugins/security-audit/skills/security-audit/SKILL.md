...
### 自動検知トリガー
- ship workflow 実行時（ship workflowが存在する場合、そのセキュリティステップで自動実行される）
...
- 認証情報ファイル（Config の `secret_files` で指定。デフォルト: `.env`, `*.token`, `*.cookie`）
...
```
出力先: Config の REPORT_DIR + REPORT_NAME で指定されたパス
デフォルト: docs/security/AUDIT_YYYY-MM-DD.md
```
...
レポート: {REPORT_DIR}/AUDIT_YYYY-MM-DD.md
即時対応が必要: X件
```
...
- ship workflowから呼ばれた場合、CRITICAL があればマージを拒否する
...
| 全ファイル網羅 | security best practice | 修正時は対象ディレクトリの全スクリプトをGlobで列挙し、パターンGrepで見落としゼロを目指す |
...
| シークレット | secret_files | `.env`, `*.token`, `*.cookie` | .gitignoreに含めるべき認証情報ファイルパターン |
...
| 状況 | 対応 |
|------|------|
| CRITICALが3件以上 | ユーザーに確認: 「CRITICAL脆弱性が{N}件検出。即時修正の優先順位を決めてください」 |
| 本番環境にシークレット露出の疑い | ユーザーに確認: 「本番のキーローテーションが必要な可能性があります。即時対応しますか？」 |
| --pentestで実環境テスト実行前 | ユーザーに確認: 「以下のテストコマンドを実行してよいですか？[コマンド一覧]」 |
| 修正によりサービス停止の可能性 | ユーザーに確認: 「この修正はダウンタイムを伴います。メンテナンス時間を設定しますか？」 |

## 合成可能性

| 連携スキル | 関係 | トリガー |
|-----------|------|---------|
| ship (if available) | 前工程 | ship workflowのセキュリティステップでsecurity-auditが自動起動される |
| incident-triage | 後工程 | CRITICAL/HIGH検出時にインシデントログを自動作成 |

## やらないこと

- 本番環境への直接アクセス（--pentestコマンド生成のみ。実行はユーザー承認後）
- シークレットの自動ローテーション（検出と推奨のみ。実行はユーザーが判断）
...
| ship (if available) | 前工程 | ship workflowのセキュリティステップでsecurity-auditが自動起動される |
| incident-triage | 後工程 | CRITICAL/HIGH検出時にインシデントログを自動作成 |
| rapid-build | 呼び出し元 | rapid-buildのセキュリティステップで包括的スキャンのために呼び出される |
...
他社は Config セクションの `REPORT_DIR`, `BLOCK_SEVERITY`, `SEMGREP_CONFIG`, `secret_files` を自社環境に変更するだけで使える。
