...
#### 2a: シークレットパターンスキャン（ソースコード + ドキュメント）

**対象**: ソースコード(.ts/.js/.py/.go) **AND** ドキュメント(.md/.yaml/.json/.env.example)
- `.md`ファイルにAPIキーが平文で書かれるケースが実際に発生した（2026-03-26インシデント）
- `.env`ファイルはnoenv-guardルールで読まないが、パターンスキャンは実行する
...
#### 2d: インフラ秘密チェック（Cloud Run / Secret Manager）

**対象**: GCPプロジェクトにCloud Runサービスがある場合に実行。
gcloudが使えない場合はスキップして報告。

```bash
# 1. Cloud Run環境変数に平文秘密がないか確認
CLOUDSDK_PYTHON=/opt/homebrew/bin/python3.13 ~/google-cloud-sdk/bin/gcloud run services describe <SERVICE> \
  --region=<REGION> --project=<PROJECT> \
  --format="yaml(spec.template.spec.containers[0].env)"
```

| チェック | PASS条件 | FAIL時の重要度 |
|---------|---------|-------------|
| APIキーが`value:`（平文）でなく`valueFrom.secretKeyRef`か | 全キーがsecretKeyRef | CRITICAL |
| `ALLOW_SIMPLE_AUTH=true`等のfail-open設定がないか | 本番では`false`またはunset | HIGH |
| SLACK_BOT_TOKEN等の秘密がenv varにないか | secretKeyRef経由 | CRITICAL |
| 不要な環境変数（旧デプロイの残骸）がないか | 使用中のコードで参照あり | MEDIUM |

```bash
# 2. Secret Managerの秘密が適切に管理されているか
CLOUDSDK_PYTHON=/opt/homebrew/bin/python3.13 ~/google-cloud-sdk/bin/gcloud secrets list --project=<PROJECT>
```

| チェック | PASS条件 | FAIL時の重要度 |
|---------|---------|-------------|
| コードが`process.env.X`で参照する秘密がSecret Managerにあるか | 全て登録済み | HIGH |
| 期限切れのSAキーがないか | 有効期限内 | MEDIUM |

**サービス/プロジェクト情報の取得**: 対象ディレクトリの`deploy.sh`、`env-vars.yaml`、`Dockerfile`からSERVICE名・PROJECT_ID・REGIONを自動検出。見つからなければownerに確認。

> 背景: 2026-03-26。GEMINI_API_KEYがCloud Runの環境変数に平文で設定されていたが、
> Step 2aのソースコードgrepでは検出できなかった。PLANドキュメントにも平文で記載。
...
- `/ship-it` から呼ばれた場合、CRITICAL があればマージを拒否する
...
### DELEGATE連携（推奨 — なくても動作する）

以下のユーザーレベルスキルがインストールされていれば、各Phaseで自動的に呼び出して精度を向上させる。
インストールされていなくても、自前のgrep/AI分析で動作する。

| スキル | Phase | 効果 | 不在時のフォールバック |
|--------|-------|------|---------------------|
| audit-context-building | Phase 0 | 行レベルのコード分析コンテキスト構築 | Phase 0スキップ、Phase 1から開始 |
| insecure-defaults | Phase 1 | fail-open設定・ハードコードシークレット検出 | 自前パターンgrep |
| sharp-edges | Phase 1 | 危険API・フットガン設計検出 | 自前パターンgrep |
| code-maturity-assessor | Phase 2 | 9カテゴリ成熟度スコアカード | スキップ |
| variant-analysis | Phase 3 | 発見した脆弱性の類似パターン検索 | スキップ |
...
| skill-upgrade | 呼び出し元 | skill-upgradeのQ11で配布安全性チェックのために呼び出される |
